...
export default function Biblioteca() {
  ...
  const loadData = async () => {
    await Promise.all([loadArticles(), loadBanners(), loadCategories(), loadAuthors()]);
  };
  ...
  const loadArticles = async () => {
    try {
      ...
      if (articlesError) {
        ...
        setArticles(articlesWithAuthors as any);
      }
    } catch (error) {
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
              ...
              {bannerSlides.map((slide, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex items-end">
                    <div className="container-fixed px-4 pb-12">
                      <div className="max-w-2xl">
                        ...
                        {slide.slug && (
                          <a
                            href={`/artigo/${slide.slug}`}
                            className="inline-block bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-2 rounded font-bold"
                          >
                            Leia Mais
                          </a>
                        )}
        ...
        <div className="container-fixed px-4 py-12">
          <h1 className="text-4xl font-bold text-foreground mb-2">Biblioteca</h1>
          <p className="text-muted-foreground mb-8 font-normal italic">
            Biblioteca de documentos e materiais de referência.
          </p>
          ...
          <div className="mb-8 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              {filteredArticles.length} {filteredArticles.length === 1 ? "documento encontrado" : "documentos encontrados"}
            </p>
          </div>

          {filteredArticles.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              <p>{articles.length === 0 ? "Nenhum conteúdo na biblioteca ainda." : "Nenhum documento encontrado com os filtros selecionados."}</p>
            </div>
          ) : (
